 ...
 * place, so `ActivityPicker` and `Game` both read from here instead of each carrying their own
 * list that could drift out of sync.
 *
 * Every `build` function shares one contract: `(roster, { recent }) => { ...roundData,
 * recentIds }`. That's what lets `Game` stay activity-agnostic — it never inspects a round's
 * shape, only forwards it to the matching `Component` and folds `recentIds` into the exclusion
 * list.
 */
...
/** Iteration order for the picker tray and for "mix"'s random pick. */
